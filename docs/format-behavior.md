...
- M8b exports allowlisted structured facts only. Raw XML, Java bodies, MAP literal values, arbitrary
  ...
  not serialized.
- M8c adds `business-result.v2`, generated only by `wm-doc enrich-business` from an existing
  `business-context.v1` `context.json`. It does not change `analysis.v8`, `scope.v1`, or
  `business-context.v1`.
- M8c sends Ollama `/api/chat` requests with `stream:false`, JSON schema `format`,
  `temperature=0`, `num_predict` bounded by the app, and a deterministic prompt envelope. The
  `format` schema is the small internal `business-draft.v1` contract, not the final result schema.
  Raw prompts, raw model responses, chain-of-thought, provider URLs, and provider diagnostics are
  not persisted.
- `business-result.v2` records app-normalized model claims, unknowns, limitations, conflicts, safe
  provider/model provenance, validation counts, and source context hash. The app computes stable
  result, claim, unknown, limitation, and conflict IDs after validation. The model never generates
  final IDs, final status, final provenance, validation metadata, cache metadata, or confidence
  enum values.
- Draft `claims` map to `SUPPORTED`; draft `inferences` map to `INFERRED`; draft `unknowns` stay
  unknowns; draft `limitations` are merged with deterministic source limitations.
- `SUPPORTED` and `INFERRED` claims must cite existing context evidence IDs, and every
  identifier a claim names must appear in the evidence it cites; ungrounded claims are discarded
  and reported as an `UNGROUNDED_CLAIM_DISCARDED` limitation. `UNKNOWN` claims may
  omit evidence only when the absence itself is the reported fact. Evidence IDs are checked exactly,
  and incompatible evidence types reject the whole response rather than silently dropping claims.
- M8c writes only `result.json` and `index.md` in the requested business output directory. The pair
  is published atomically and unrelated files in that directory are preserved. The Markdown page is
  generated by the app from validated JSON and labels claims as model-generated and evidence-checked.
- M8c cache entries contain only validated normalized `business-result.v2` JSON. Cache keys include
  the raw context hash, provider kind, model name, model digest when available, prompt version,
