...
- M8b exports allowlisted structured facts only. Raw XML, Java bodies, MAP literal values, arbitrary
  ...
  not serialized.
- M8c adds `business-result.v1`, generated only by `wm-doc enrich-business` from an existing
...
- `business-result.v1` records app-normalized model claims, unknowns, limitations, conflicts, safe
  ...
  enum values.
- Draft `claims` map to `CONFIRMED`; draft `inferences` map to `INFERRED`; draft `unknowns` stay
  unknowns; draft `limitations` are merged with deterministic source limitations.
- `CONFIRMED` and `INFERRED` claims must cite existing context evidence IDs. `UNKNOWN` claims may
...
- M8c cache entries contain only validated normalized `business-result.v1` JSON. Cache keys include
